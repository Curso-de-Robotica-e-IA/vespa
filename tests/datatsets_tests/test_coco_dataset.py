...
def test_coco_train_transforms_boxes(create_coco_annotations):
    ...
    _, target = dataset[0]
    assert len(target["boxes"]) > 0, "As bounding boxes não foram transformadas corretamente no treino."
...
def test_coco_test_transforms_no_bboxes(create_coco_annotations):
    ...
    _, target = dataset[0]
    assert "boxes" in target, "Bounding boxes devem ser incluídas no target, mesmo sem transformações."
