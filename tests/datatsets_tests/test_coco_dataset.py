...
def test_coco_dataset_length(create_coco_annotations):
    ...
    dataset = COCODataset(
        root_dir=dataset_path, ann_file=annotations_path, transforms=None
    )
    assert len(dataset) == len, 'Tamanho do dataset COCO está incorreto.'  # noqa
...
def test_coco_dataset_image_dimensions(create_coco_annotations):
    ...
    assert img.shape == (100, 100, 3), (
        f'Dimensão da imagem no COCODataset está incorreta: {img.shape}'
    )  # noqa
...
def test_coco_train_transforms_boxes(create_coco_annotations):
    ...
    _, target = dataset[0]
    assert len(target['boxes']) > 0, (
        'As bounding boxes não foram transformadas corretamente no treino.'
    )  # noqa
