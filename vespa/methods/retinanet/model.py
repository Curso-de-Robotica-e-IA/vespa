...
from torch.optim import Adam, SGD, Adadelta, Adagrad, LBFGS, RMSprop
...
class RetinaNet(Module):
    ...
    def train(self, train_dataset, val_dataset, batch_size, epochs=20, device=0):
        # Load model on gpu
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (images, targets) in enumerate(train_loader):
                ...
                print(f"Epoch [{epoch+1}/{epochs}], Batch [{batch_idx+1}/{len(train_loader)}] Loss: {losses.item()}\033[0m",  end=' ', flush=True)
    ...
    def configure_optimzer(self, optim='adam', lr=0.0001, weight_decay=0.0001):
        ...
        Parameters:
        optim (str): The type of optimizer to use. Options are 'adam', 'sgd', 'adadelta', 'adagrad', 'lbfgs', and 'rmsprop'. Default is 'adam'.
        lr (float): The learning rate for the optimizer. Default is 0.0001.
        weight_decay (float): The weight decay (L2 penalty) for the optimizer. Default is 0.0001.
        Raises:
        ...
        if optim == 'adam':
            ...
            self.optimizer = Adadelta(self.model.parameters(), lr=lr, weight_decay=weight_decay)
        elif optim == 'adagrad':
            self.optimizer = Adagrad(self.model.parameters(), lr=lr, weight_decay=weight_decay)
        elif optim == 'lbfgs':
            self.optimizer = LBFGS(self.model.parameters(), lr=lr, weight_decay=weight_decay)
        elif optim == 'rmsprop':
            self.optimizer = RMSprop(self.model.parameters(), lr=lr, weight_decay=weight_decay)
        else:
