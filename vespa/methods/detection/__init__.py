...
Usage:
    - Train a model:
        ...
        train_model("yolo", "path/to/dataset", "yolo", batch_size=8, epochs=25, device="cuda")

    - Validate a model:
        from vespa.methods.detection import validate_model
        validate_model("rcnn", "path/to/dataset", "yolo", batch_size=8, device="cuda")

    - Test a model:
        from vespa.methods.detection import test_model
        test_model("retinanet", "path/to/dataset", "yolo", batch_size=8, device="cuda")
...
Functions:
    - train_model(model_name: str, dataset_path: str, dataset_format: str, **dataset_kwargs): Trains a specified detection model.
    - validate_model(model_name: str, dataset_path: str, dataset_format: str, **dataset_kwargs): Validates a specified detection model.
    - test_model(model_name: str, dataset_path: str, dataset_format: str, **dataset_kwargs): Tests a specified detection model.
...
from vespa.datasets.detection_formats.pascal_voc.pascal_voc_transforms import get_pascal_voc_train_transforms, get_pascal_voc_test_transforms
...
def get_num_classes(dataset_path: str, dataset_format: str) -> int:
    ...
    if dataset_format == "yolo":
        ...
        if not os.path.exists(yaml_file):
            raise FileNotFoundError(f"YOLO dataset config file not found: {yaml_file}")
        ...
        if not class_names:
            raise ValueError("No class names found in YOLO dataset configuration.")
        ...
        if not os.path.exists(json_file):
            raise FileNotFoundError(f"COCO dataset config file not found: {json_file}")
        ...
        if not os.path.exists(xml_file):
            raise FileNotFoundError(f"Pascal VOC dataset config file not found: {xml_file}")
...
def train_model(model_name: str, dataset_path: str, dataset_format: str, **dataset_kwargs):
    """
    ...
    train_dataset = load_dataset(
        dataset_path, dataset_format, model_name=model_name, transforms=train_transforms, **dataset_kwargs
    )
    
    num_classes = get_num_classes(dataset_path, dataset_format)

    if num_classes is None:
        raise ValueError(f"Failed to determine the number of classes for dataset: {dataset_path}")
...
def validate_model(model_name: str, dataset_path: str, dataset_format: str, **dataset_kwargs):
    """
...
__all__ = ["ONNX_YOLO", "RCNN", "RetinaNet", "train_model", "validate_model", "available_models", "run_inference"]
