...
This module provides dataset loaders for object detection
models in various formats, including YOLO, COCO, and Pascal VOC.

### **Usage:**
#### **Load a dataset:**
```python
from vespa.datasets.detection import (
    load_dataset,
)
...
def load_dataset(
    dataset_path: str, dataset_format: str, model_name, transforms, **kwargs
):
    """
    ...
    if dataset_format == 'yolo':
        ...
        for key in required_keys:
            if key not in kwargs:
                raise ValueError(
                    f"Missing required argument '{key}' for YOLODataset."
                )  # noqa
        ...
        if 'txt_file' not in kwargs:
            raise ValueError(
                "Missing required argument 'txt_file' for COCODataset."
            )  # noqa
...
__all__ = ['COCODataset', 'PascalVOCDataset', 'YOLODataset', 'load_dataset']
